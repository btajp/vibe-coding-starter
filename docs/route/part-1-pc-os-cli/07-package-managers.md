...
第0部では、CodexまたはClaude Codeを入れるために使いました。
...
グローバルインストールすると、ターミナルから `codex` や `claude` のようなコマンドとして実行できるようになります。
...
CodexやClaude Codeは、npmでインストールできるCLIツールとして提供されています。
...
第0部でNode.js / npmを入れたのは、AIエージェントをターミナルから起動するための土台を作るためでした。
...
インストールや更新を追加で行う場合は、何を入れたのかをメモしておきます。
ただし、環境に入れたツールそのものはGitのcommit対象ではありません。
