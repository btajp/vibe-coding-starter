...
どこを探すかを決める手がかりがPATHです。

PATHは、コマンドを探すディレクトリの一覧です。
...
この章では、設定ファイルの確認はしますが、教材リポジトリのファイルは編集しません。
通常はcommitするものはありません。

もしAIに設定ファイルの編集を頼む場合は、教材リポジトリではなく自分のホームディレクトリのファイルが変わります。
その変更はGitのcommit対象ではありません。
実行前に内容をよく確認してください。
